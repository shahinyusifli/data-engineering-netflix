...
- c. Which country has the potential for improving earnings if Netflix starts charging subscribers an additional fee for sharing Netflix households outside of their own? This involves examining profits generated by multiple households linked to a single contract. For two selected households, their profits could be doubled to if we change policy to sharing user. We can see result with 
 Query:
 ``` sql
 SELECT
