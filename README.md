...
This involves examining profits generated by multiple households linked to a single contract. For two selected households, their profits could be doubled to if we change the policy to sharing users. We can see the result with this SQL query:
 ``` sql
with result_of_1_more_households as (SELECT
   da.country,
   SUM(CASE WHEN fs.household_profile_ind >= 1 THEN ds.revenue ELSE 0 END) OVER w AS profit_more_than_1_household,
   SUM(CASE WHEN fs.household_profile_ind = 1 THEN ds.revenue ELSE 0 END)  OVER w AS profit_1_household
FROM
   gold.fct_sales fs
JOIN
   gold.dim_account da ON fs.account_id = da.id
JOIN
   gold.dim_subscription ds ON fs.subscription_id = ds.id
WINDOW w AS (PARTITION BY da.country)), 
earnable_profits AS (SELECT
	country,
	profit_more_than_1_household,
	profit_1_household,
	profit_more_than_1_household * 2 as ernable_profit,
	row_number() OVER (ORDER BY profit_more_than_1_household DESC) AS row_number_desc 
FROM 
	result_of_1_more_households
)

SELECT 
		country AS "Country",
		profit_more_than_1_household AS "Profit of more than 1 household", 
		profit_1_household AS "Profit of 1 household",
		ernable_profit AS "Earnable profit"
FROM 
		earnable_profits 
WHERE 
		row_number_desc = 1;
